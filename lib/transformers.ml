...
let lift {M : Monad} m = StateT (fun s -> M.fmap (fun x -> (s, x)) m)
...
module type MonadTrans = sig
  module T : functor {M: Monad} -> Monad
  val lift : {M: Monad} -> 'a M.t -> 'a TM.t
end

implicit module StateT_Trans {S: Any} : MonadTrans = struct
  x
end
